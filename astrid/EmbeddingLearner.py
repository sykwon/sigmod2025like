...
class EmbeddingCNNNetwork(nn.Module):
    def __init__(self, string_helper, configs):
        ...
        self.flat_size = self.max_string_length // 8 * \
            self.alphabet_size * self.channel_size
        self.fc1 = nn.Linear(self.flat_size, self.embedding_dimension)

    # Perform convoution to compute the embedding
    def forward(self, x):  # [batch_size, alphabet_size, length]
        N = len(x)
...
def train_embedding_model(configs, train_loader, string_helper):
    ...
    for epoch in tqdm(range(configs.num_epochs), desc="Epochs"):
        ...
        print("Epoch: {}/{} - Mean Running Loss: {:.6f}".format(epoch +
              1, configs.num_epochs, np.mean(running_loss)))
