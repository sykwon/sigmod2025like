...
import random
import torch
from astrid.string_dataset_helpers import StringDatasetHelper
...
AstridEmbedLearnerConfigs = namedtuple(
    "AstridEmbedLearnerConfigs",
    [
        "embedding_dimension",
        "batch_size",
        "num_epochs",
...
StringFrequencyConfigs = namedtuple(
    "StringFrequencyConfigs",
...
SelectivityEstimatorEachConfigs = namedtuple('SelectivityEstimatorConfigs',
                                             ['embedding_dimension', 'batch_size', 'num_epochs', 'device', 'lr',
                                              'min_val', 'max_val',
                                              'embedding_model_file_name', 'selectivity_model_file_name'])

SelectivityEstimatorConfigs = namedtuple(
    "SelectivityEstimatorConfigs",
    [
        "embedding_dimension",
        "batch_size",
        "num_epochs",
...
def normalize_labels(labels, min_val=None, max_val=None):
    labels = np.array([np.log(float(max(l, 1))) for l in labels])
    if min_val is None:
        min_val = labels.min()
    if max_val is None:
        max_val = labels.max()
...
# Copied from learned cardinalities paper
def qerror_loss(preds=None, targets=None, min_val=0.0, max_val=1.0, reduction=True):
    qerror = []
