#ifndef A3652E50_BFCB_466B_8CFA_00A9881F7B83
#define A3652E50_BFCB_466B_8CFA_00A9881F7B83
...
public:
    ...
    void set_subtrie(TrieNode<_CharT>* src_root) {
        ...
        for (auto key : keys) {
            ...
            if (IS_UWILDCARD(*dest_child->s_str)) {
                this->has_wildcard_child = true;
            }
...
public:
    ...
    void add_strings(u32string* queries, int n_qrys) {
        _nodeT* curr_node;
        _stringT query;
        for (int rid = 0; rid < n_qrys; rid++) {
            query = queries[rid];
            // cout<< "node rid" << endl;
    ...
    int add_all_prefixes(u32string* queries, int n_qrys) {
        ...
        for (int rid = 0; rid < n_qrys; rid++) {
            ...
            // cout<< "node rid" << endl;
            //  cout<< "query: " <<query<<endl;
            curr_node = this->add_all_prefixes(query);
            curr_node->set_active_node(rid);
            // cout<< "rid: "<<curr_node->rid << "active: " <<curr_node->is_active() << endl;
        }
...
#endif /* A3652E50_BFCB_466B_8CFA_00A9881F7B83 */
