from src.E2E.feature_extraction.database_loader import *
from src.E2E.feature_extraction.plan_features import *
from src.E2E.feature_extraction.sample_bitmap import *
    ...
    with open(input_path, 'r') as ff:
        with open(output_path, 'w') as f:
            for count, plan in enumerate(ff.readlines()):
                print(count)
                # rewrite values in 'seq'
                parsed_plan = json.loads(plan)
                nodes_with_sample = []
                for node in parsed_plan['seq']:  # list of nodes
                    bitmap_filter = []
                    bitmap_index = []
                    bitmap_other = []
                    if node != None and 'condition' in node:  # type(node): dictionary
                        predicates = node['condition']  # list of condition_ops
                        if len(predicates) > 0:
                            root = TreeNode(predicates[0], None)
                            if len(predicates) > 1:
                                recover_tree(predicates[1:], root)  # build tree
