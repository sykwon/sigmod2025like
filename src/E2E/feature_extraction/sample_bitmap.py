...
def get_bitmap(root, data, sample, sample_num):
    ...
    if predicate is not None and predicate['op_type'] == 'Compare': # Compare type
        # prediate['op_type'] = 'Compare'
        # prediate['left_value'] = 'aka_title.title'
        # prediate['right_value'] = 'Dinos in'
        ...
        if result is None:
            ...
            for value in sample[table_name][column].tolist(): # sampled values
                if isSelected(value, predicate, dtype):
                    vec.append(1)
                else:
                    vec.append(0)
            for i in range(len(vec), sample_num): # padding
                vec.append(0)
        elif not predicate['right_value'].split('.')[0] in data: # predicate['right_value'] table.column
            dtype = data[table_name].dtypes[column]
            ...
            for i in range(len(vec), sample_num):
                vec.append(0)
        return vec
    elif predicate is not None and predicate['op_type'] == 'Bool': # Boolean type
        bitmap = []
...
def isSelected(row_value, predicate, dtype):
    if dtype == 'int64':
        ...
        if op == '=':
            if value.startswith('__LIKE__'): # LIKE query matching with samples
                v = value[8:]
                pattern = r'^'
