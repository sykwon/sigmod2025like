from src.E2E.plan_encoding.encoding_predicates import *
...
def encode_node_job(node, parameters):
    ...
    condition1_vec = np.array([[0 for _ in range(parameters.condition_op_dim)]
                              for _ in range(parameters.condition_max_num)])
    condition2_vec = np.array([[0 for _ in range(parameters.condition_op_dim)]
                              for _ in range(parameters.condition_max_num)])
    # Samples Starts
