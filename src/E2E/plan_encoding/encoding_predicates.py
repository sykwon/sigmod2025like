...
from src.E2E.plan_encoding.meta_info import *


def get_representation(value, word_vectors, hash_size=500):
    # value: title_Din or title_in
    # word vector size: 500
    # hash vector size: 500
    # word + hash vector size: 1000

    # print(f"{value = }")
    # for length in range(len(value), 0, -1):
    #     prefix_value = value[:length]
    #     suffix_value = value[-length:]
    #     if prefix_value in word_vectors:
    #         print("O ", end="")
    #     else:
    #         print("X ", end="")
    #     print(f"{prefix_value = :10s}", end="")
    #     if suffix_value in word_vectors:
    #         print("O ", end="")
    #     else:
    #         print("X ", end="")
    #     print(f"{suffix_value = }")
    ...
    for length in range(len(value), 0, -1):
        prefix_value = value[:length]
        if prefix_value in word_vectors:
            embedded_result = np.array(list(word_vectors[prefix_value]))
            # print(f"{prefix_value = }")
            break
        suffix_value = value[-length:]
        if suffix_value in word_vectors:
            embedded_result = np.array(list(word_vectors[suffix_value]))
            # print(f"{suffix_value= }")
            break
    ...
    hash_result = np.array([0.0 for _ in range(hash_size)])
    for t in value:
        hash_result[hash(t) % hash_size] = 1.0
    # print(f"{np.average(embedded_result), np.average(hash_result) = }")
...
def encode_condition_op(condition_op, relation_name, index_name, parameters):
    ...
    if condition_op is None:
        ...
        if re.match(r'^[a-z][a-zA-Z0-9_]*\.[a-z][a-zA-Z0-9_]*$', right_value) is not None and right_value.split('.')[
            ...
            operator_vec = [0 for _ in range(parameters.compare_ops_total_num)]
            operator_vec[operator_idx - 1] = 1
            right_value = right_value.strip('\'')[8:]  # lstrip __LIKE__
