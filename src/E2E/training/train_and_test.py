from src.E2E.plan_encoding.spilling2disk import *
from src.E2E.training.representation_model import *
from src.E2E.training.vector_loader import *
...
def detached_qerror_loss_without_reduction(preds, targets, mini, maxi):
    qerror_list = []
    preds = unnormalize(preds, mini, maxi)
    targets = unnormalize(targets, mini, maxi)
    for i in range(len(targets)):
        if (preds[i] > targets[i]).cpu().data.numpy()[0]:
            qerror = preds[i] / targets[i]
        else:
            qerror = targets[i] / preds[i]
        qerror_list.append(qerror.item())
    return qerror_list
